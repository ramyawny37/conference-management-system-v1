...
const cacheRevision='reservations-root-boundary-v1';
...
const productionCacheRevision='production-3-6-0-release-v6';
...
const pwaAssetRevision='pwa-update-state-consistency-v1';
...
assert(pwa.includes('getActionableWaitingWorker'),'update UI must use the actual waiting worker');
assert(!pwa.includes('registration.waiting || newWorker'),'installing workers must not create actionable UI');
