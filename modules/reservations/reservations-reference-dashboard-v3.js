...
function render(){
 var workspace=doc.getElementById('reservationsWorkspace');
 if(!workspace)return;
 var route=(global.ApplicationRouting&&global.ApplicationRouting.getLogicalPathname?global.ApplicationRouting.getLogicalPathname():global.location.hash||'');
 if(String(route).indexOf('/reservations')!==0&&String(global.location.hash||'').indexOf('reservations')<0)return;
 if(workspace.querySelector('[data-reservations-reference-v3]'))return;
 workspace.innerHTML='<main class="rv3-dashboard" data-reservations-reference-v3 aria-label="لوحة الحجوزات الجديدة">\
...
<section class="rv3-lower">\
 ...
 workspace.classList.add('reservations-reference-v3-host');
}
function schedule(){global.setTimeout(render,0);global.setTimeout(render,120);global.setTimeout(render,500);}
doc.addEventListener('DOMContentLoaded',schedule);
global.addEventListener('hashchange',schedule);
doc.addEventListener('click',function(e){if(e.target&&e.target.closest&&e.target.closest('[data-platform-module="reservations"]'))global.setTimeout(schedule,80);});
})(window);