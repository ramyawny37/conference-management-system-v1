...
-- Restore the established multi-device authorization invariant.
-- Approval is additive: an already-approved active device does not force replacement.
-- All target-state, account, binding, actor, revoke, replace, idempotency and audit guards
-- remain owned by the canonical function below.
