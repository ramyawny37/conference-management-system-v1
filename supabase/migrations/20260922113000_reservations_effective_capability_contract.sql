...
-- This is advisory UI state only; every read/mutation remains server-authorized.
...
begin
  ...
  return jsonb_build_object(
    'permissions',coalesce((
      ...
      from public.module_permission_catalog p
      where p.module_key='reservations' and p.status='active' and (
        v_is_owner or exists(
          select 1 from public.module_permission_grants g
          where g.user_id=v_actor and g.module_key='reservations'
            and g.permission_key=p.permission_key and g.revoked_at is null
            and (
              (g.resource_type is null and g.resource_id is null)
              or (g.resource_type='event' and exists(
                select 1 from reservations.events e
                where e.id::text=g.resource_id
...
begin
  if p_operation='get_effective_capabilities' then
    return reservations_private.effective_capabilities(p_device_id,p_args);
  end if;
