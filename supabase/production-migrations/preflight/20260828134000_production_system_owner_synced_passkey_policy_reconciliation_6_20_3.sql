-- Independent read-only predecessor-contract preflight for 20260828134000.
with latest as (select version,name from supabase_migrations.schema_migrations order by version desc limit 1)
select case when (select version='20260828133000' and name='production_credential_enrollment_expiry_default_alignment_6_20_2' from latest)
    and exists(select 1 from supabase_migrations.schema_migrations where version='20260828133000'
      and name='production_credential_enrollment_expiry_default_alignment_6_20_2' and md5(coalesce(array_to_string(statements,E'\n'),''))='1161e049346579b636221edf178cf4f6')
    ...
    where p.oid=to_regprocedure('public.begin_system_owner_credential_enrollment(uuid,uuid,uuid,text,text,text,bytea,uuid,bytea)')))
