-- Independent read-only predecessor-contract preflight for 20260828133000.
with latest as (select version,name from supabase_migrations.schema_migrations order by version desc limit 1)
select case when (select version='20260828132000' and name='production_credential_enrollment_challenge_expiry_reconciliation_6_20_1' from latest)
    and exists(select 1 from supabase_migrations.schema_migrations where version='20260828132000'
      and name='production_credential_enrollment_challenge_expiry_reconciliation_6_20_1' and md5(coalesce(array_to_string(statements,E'\n'),''))='a57e51772d13635c4cbacb8d183cca7a')
    ...
    regexp_replace(pg_get_functiondef(to_regprocedure('public.begin_system_owner_credential_enrollment(uuid,uuid,uuid,text,text,text,bytea,uuid,bytea)')),'[[:space:]]+','','g'))>0)
