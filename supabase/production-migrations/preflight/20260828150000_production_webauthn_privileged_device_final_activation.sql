-- Independent read-only predecessor-contract preflight for 20260828150000.
with latest as (select version,name from supabase_migrations.schema_migrations order by version desc limit 1)
select case when (select version='20260828145000' and name='production_platform_foundation_legacy_execute_grant_reconciliation' from latest)
    and exists(select 1 from supabase_migrations.schema_migrations where version='20260828145000'
      and name='production_platform_foundation_legacy_execute_grant_reconciliation' and md5(coalesce(array_to_string(statements,E'\n'),''))='961164b7ebe303f1c6873d25bdf4348e')
    and ((
    ...
    and (with expected(ordinal,version,name,digest) as (values (1,'20260828130000','production_platform_foundation_lineage_bridge','071d06a5cdbbee8c7a441fd944d2d0c6'),
    ...
    and not exists(select 1 from (values
      ...
      where acl.privilege_type='EXECUTE' and (required.role_name='PUBLIC' and acl.grantee=0
        or required.role_name<>'PUBLIC' and acl.grantee=(select oid from pg_roles where rolname=required.role_name)))))
    and (not exists(select 1 from public.device_security_credentials)
