-- Independent read-only predecessor-contract preflight for 20260828140000.
with latest as (select version,name from supabase_migrations.schema_migrations order by version desc limit 1)
select case when (select version='20260828135000' and name='production_system_owner_synced_passkey_policy_full_reconciliation_6_20_4' from latest)
    and exists(select 1 from supabase_migrations.schema_migrations where version='20260828135000'
      and name='production_system_owner_synced_passkey_policy_full_reconciliation_6_20_4' and md5(coalesce(array_to_string(statements,E'\n'),''))='94f58043da178dacd210bb61908c44a3')
