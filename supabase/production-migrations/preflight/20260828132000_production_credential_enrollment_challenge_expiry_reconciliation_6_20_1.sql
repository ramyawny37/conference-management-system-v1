-- Independent read-only predecessor-contract preflight for 20260828132000.
with latest as (select version,name from supabase_migrations.schema_migrations order by version desc limit 1)
select case when (select version='20260828131000' and name='production_platform_privileged_device_administration_structural_6_20_0' from latest)
    and exists(select 1 from supabase_migrations.schema_migrations where version='20260828131000'
      and name='production_platform_privileged_device_administration_structural_6_20_0' and md5(coalesce(array_to_string(statements,E'\n'),''))='f0026f5e23a3c36e88646c26344fea88')
