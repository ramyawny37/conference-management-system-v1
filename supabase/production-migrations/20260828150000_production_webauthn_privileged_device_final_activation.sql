...
-- Semantic release: 6.20-final-activation; intentional delta: PRODUCTION_FINAL_ACTIVATION.
do $production_gate$
...
begin
  ...
  if latest.version is distinct from '20260828145000'
    or latest.name is distinct from 'production_platform_foundation_legacy_execute_grant_reconciliation' then
  ...
  if not (with expected(ordinal,version,name,digest) as (values (1,'20260828130000','production_platform_foundation_lineage_bridge','071d06a5cdbbee8c7a441fd944d2d0c6'),
      (2,'20260828131000','production_platform_privileged_device_administration_structural_6_20_0','f0026f5e23a3c36e88646c26344fea88'),
      (3,'20260828132000','production_credential_enrollment_challenge_expiry_reconciliation_6_20_1','a57e51772d13635c4cbacb8d183cca7a'),
      (4,'20260828133000','production_credential_enrollment_expiry_default_alignment_6_20_2','1161e049346579b636221edf178cf4f6'),
      (5,'20260828134000','production_system_owner_synced_passkey_policy_reconciliation_6_20_3','fd8777456bc9937692ec740ab0b7d83e'),
      (6,'20260828135000','production_system_owner_synced_passkey_policy_full_reconciliation_6_20_4','94f58043da178dacd210bb61908c44a3'),
      (7,'20260828140000','production_webauthn_time_boundary_reconciliation_6_20_5','f387c81e6cd768a32d46e7b177f49478'),
      (8,'20260828141000','production_webauthn_verification_material_base64_reconciliation_6_20_6','42ccd207946ad243233ad179f7b86408'),
      (9,'20260828142000','production_pending_device_listing_source_purpose_reconciliation_6_20_7','e0bef956c4d974f642305fb5d9fe90c4'),
      (10,'20260828143000','production_pending_device_operation_purpose_qualification_reconciliation_6_20_8','234d70bc3af1a1d1d069343cd9f31a7e'),
      (11,'20260828144000','production_pending_device_operation_purpose_variable_reconciliation_6_20_9','d44194db2a86d023a04fb67e57fe95f5'),
      (12,'20260828145000','production_platform_foundation_legacy_execute_grant_reconciliation','961164b7ebe303f1c6873d25bdf4348e')), actual as (
      select row_number() over(order by version)::integer ordinal,version,name,
        md5(coalesce(array_to_string(statements,E'\n'),'')) digest
      from supabase_migrations.schema_migrations
      where version between '20260828130000' and '20260828145000')
    select count(*)=12 and bool_and(expected.ordinal is not null
    ...
    from expected full join actual using(ordinal)) then
    raise exception 'PRODUCTION_ACTIVATION_EXACT_FULL_LINEAGE_REQUIRED';
  end if;
  if not (not exists(select 1 from public.device_security_credentials)
    ...
    and not exists(select 1 from public.system_owner_device_authorization_operations)) then
...
begin
  if not ((select count(*)=9 and bool_and(c.relrowsecurity) and count(distinct c.relowner)=1
    ...
    and not exists(select 1 from (values
      ...
      where acl.privilege_type='EXECUTE' and (required.role_name='PUBLIC' and acl.grantee=0
        or required.role_name<>'PUBLIC' and acl.grantee=(select oid from pg_roles where rolname=required.role_name))))) then
...
begin
  if not (select count(*)=1 and bool_and(enabled)
    ...
    or not ((select count(*)=9 and bool_and(c.relrowsecurity) and count(distinct c.relowner)=1
      and bool_and(c.relowner=(select relowner from pg_class
        where oid=to_regclass('public.user_device_authorizations')))
      and bool_and(not exists(select 1 from pg_policy where polrelid=c.oid))
      and bool_and(not has_table_privilege('anon',c.oid,'select,insert,update,delete'))
