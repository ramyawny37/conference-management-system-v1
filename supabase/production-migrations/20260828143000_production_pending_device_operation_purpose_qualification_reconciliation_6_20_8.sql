...
-- Semantic release: 6.20.8; intentional delta: CANONICAL_SEMANTIC_EQUIVALENT.
do $production_gate$
...
begin
  ...
  if latest.version is distinct from '20260828142000'
    or latest.name is distinct from 'production_pending_device_listing_source_purpose_reconciliation_6_20_7' then
...
begin
  ...
  select * into challenge from public.device_possession_challenges challenges
    where challenges.id=p_challenge_id and challenges.user_id=p_actor_user_id
      ...
      and challenges.purpose=complete_system_owner_pending_device_operation.purpose
