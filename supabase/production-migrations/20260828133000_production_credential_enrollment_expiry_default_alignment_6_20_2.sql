...
-- Semantic release: 6.20.2; intentional delta: CANONICAL_SEMANTIC_EQUIVALENT.
do $production_gate$
...
begin
  ...
  if latest.version is distinct from '20260828132000'
    or latest.name is distinct from 'production_credential_enrollment_challenge_expiry_reconciliation_6_20_1' then
...
$production_gate$;
-- Align credential-enrollment challenge expiry with the transaction-stable
-- created_at default (now()) used by device_possession_challenges.
...
begin
  ...
  values(p_challenge_hash,p_actor_user_id,p_session_id,p_actor_device_id,null,
    ...
    lower(p_expected_rp_id),p_environment,now()+interval '2 minutes')
