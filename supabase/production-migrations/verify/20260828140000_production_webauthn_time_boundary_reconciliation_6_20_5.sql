-- Independent read-only resulting-contract verifier for 20260828140000.
select case when exists(select 1 from supabase_migrations.schema_migrations
    where version='20260828140000' and name='production_webauthn_time_boundary_reconciliation_6_20_5') and ((select count(*)=2 and bool_and(p.prosecdef)
    ...
    regexp_replace((select p.prosrc from pg_proc p
      where p.oid=to_regprocedure('public.complete_system_owner_pending_device_listing(uuid,uuid,uuid,uuid,text,uuid,bytea,uuid,bytea,bigint,text,text,jsonb)')),'[[:space:]]+','','g'))>0) then 'PASS' else 'BLOCKED' end,
